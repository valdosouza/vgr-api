...
describe('admin-login.service', () => {
  ...
  it('issues a JWT with role=admin when the password matches the stored bcrypt hash', async () => {
    ...
    mockedRepository.findAdminAccountByEmail.mockResolvedValue({ id: 1, email: 'valdo@vgr.com.br', passwordHash, active: 'S', sessionVersion: 1, failedLoginCount: 0 })
  ...
  it('rejects a deactivated account with the same generic 401 (no account enumeration)', async () => {
    const passwordHash = await bcrypt.hash('teste', 10)
    mockedRepository.findAdminAccountByEmail.mockResolvedValue({ id: 1, email: 'valdo@vgr.com.br', passwordHash, active: 'N', sessionVersion: 1, failedLoginCount: 0 })
  ...
  it('never reveals whether the email or the password was wrong (same message/status for both)', async () => {
    ...
    const passwordHash = await bcrypt.hash('teste', 10)
    mockedRepository.findAdminAccountByEmail.mockResolvedValueOnce({ id: 1, email: 'valdo@vgr.com.br', passwordHash, active: 'S', sessionVersion: 1, failedLoginCount: 0 })
