...
| DirectionSighting | Aggregate Root (append-only) | Missing from the original catalog — added here (2026-09-04). One per logged sighting; never updated or deleted, never revoked/edited (append-only signal, plano-direction-sightings.md §6). **Implemented 2026-09-04**: `tb_direction_sighting` (migration 047) — `direction`, the RESOLVED `SightingWeight` AT LOGGING TIME (never recomputed later), `account_id` (NULL if anonymous), `client_key` UNIQUE (replay-safety ONLY — decision 28/137 — unlike `tb_report`/`tb_panic_alert` it never doubles as a bearer secret for a later action, since a sighting is never resolved/edited) | *see below* |
| DirectionEstimate | Aggregate Root | Weight recalculates on every new Sighting; never goes negative. **Implemented 2026-09-04 — see `docs/feature/direction-sightings.md`**: no single "probability map" field — `tb_direction_estimate` (migration 047) holds one row per (report, direction) with `total_weight`/`sighting_count`, updated INCREMENTALLY (`ON DUPLICATE KEY UPDATE`) so reconciliation stays O(1); the winning direction is picked by `pickWinningDirection` (`shared/direction-sighting/direction-estimate.ts`, decision 26), never exposed as a distribution (203); the disclosure floor (202, env `DIRECTION_SIGHTING_MIN_COUNT`) gates every READ path, never the actor's own synchronous write response (22) | *see below* |
...
| HelpType | Help Matching | One of the fixed list from decision 10. **Amended 2026-09-11 (HT1, decision 208)**: a HelpOffer carries a SET of them (1..5, no repeats) — `shared/help-offer/help-types.ts`, child table `tb_help_offer_type` (209) | *see below* |
| Direction | Direction Sighting Aggregation | Compass point enum (N/S/E/W/NE/NW/SE/SW). **Implemented 2026-09-04** as `DIRECTIONS`/`Direction` in `shared/direction-sighting/direction-estimate.ts` (order: N/NE/E/SE/S/SW/W/NW) — promoted to shared like `Category`/`Subject`, since direction-sightings/reports/help-matching all need it; `direction-sightings.interface.ts` re-exports it (mirrors `reports.interface.ts`'s own re-export of `Category`/`Subject`) | *see below* |
| SightingWeight | Direction Sighting Aggregation | Float 0–1; lower for anonymous helper (decision 27). **Implemented 2026-09-04**: not a standalone class — `directionSightingConfig()` (`shared/config/env.ts`) resolves `SIGHTING_WEIGHT_IDENTIFIED` (default 1.0) / `SIGHTING_WEIGHT_ANONYMOUS` (default 0.5, decision 205) at LOGGING time; the resolved number is stored on `tb_direction_sighting.weight`, never recomputed if the env var changes later. Decision 206: NOT scaled by helper reputation (`ratings`) this round — closes decision 189's open promise by explicitly declining | *see below* |
...
| SubmitHelpOffer | Registers a HelpOffer with the chosen HelpTypes; rejects self-dealing (decision 20). **Amended 2026-09-11 (208/213)**: `helpTypes: HelpType[]`, never a single type. **UpdateHelpOfferTypes (211)**: the offer's own helper (matched by account — a fully anonymous offer cannot be claimed) replaces the set while the report is open; `help_offer_updated` timeline item (212) | HelpOffer, Report | *see below* |
| LogDirectionSighting | Records a Sighting synchronously and triggers reweighting (decision 22). **Implemented 2026-09-04** as `direction-sightings.service.ts logDirectionSighting`: idempotent by clientKey (28/137) → report exists (404) → category eligible 422 `DIRECTION_SIGHTING_NOT_ELIGIBLE` (201) → report open 422 `BUSINESS_RULE` → self-dealing 422 `BUSINESS_RULE`, identified-only (200, mirrors help-offers' decision-20 posture exactly) → gate `location.tracking` 451 (7/22/26) → insert (one transaction, log + O(1) aggregate upsert) → accountability for the anonymous sighter (23) → returns `{ sightingId, reportId, estimate, count }` SYNCHRONOUSLY, ungated by the floor (202 governs READ paths only) — see `POST /app-direction-sightings` | DirectionSighting, DirectionEstimate | *see below* |
| ReconcileDirectionEstimate | Recomputes weighted probability across all Sightings (decision 26, 27). **Implemented 2026-09-04** as the pure functions `pickWinningDirection`/`totalSightingCount`/`meetsDisclosureFloor` (`shared/direction-sighting/direction-estimate.ts`): the direction with the highest ACCUMULATED WEIGHT wins (never a majority vote), tie broken by whichever direction was reported FIRST for the report (deterministic); called from the write path (ungated) and from both READ facets (`reports.repository.ts`'s `getDirectionEstimateFacet`, `help-matching.repository.ts`'s batched `findDirectionEstimates`), floor-gated there (202) | DirectionEstimate, SightingWeight | *see below* |
...
| GetReportVisibility | Restricts a Resolved Report's detail/timeline/ratings to participants only (decision 50). **Amended 2026-09-19 (HT1 addendum, decision 211)**: the `participant` view carries `myOffer { helpOfferId, helpTypes }` — the viewer's own identified offer, so the app can edit its fronts; never on other views | Report, HelpOffer | *see below* |
...
```
```ts
// Amended 2026-09-11 (HT1, decisions 208/213): a SET of fronts, not one type.
function submitHelpOffer(reportId: ReportId, helperId: UserId, types: HelpType[]): HelpOffer {
  // throws SelfDealingError if helperId === report.reporterId (decision 20)
  // types: 1..5 distinct values of decision 10's list (208)
}
// Decision 211: replace the set while the report is open; only the offer's
// own helper (by account) — a fully anonymous offer has nothing to match.
function updateHelpOfferTypes(helpOfferId: HelpOfferId, helperId: UserId, types: HelpType[]): HelpOffer {
  // 404 when not the caller's offer; 422 when the report is resolved (18)
}
```
```ts
...
| HelpOfferSubmitted | SubmitHelpOffer succeeds | `{ helpOfferId, reportId, helperId, helpTypes }` — **Amended 2026-09-11 (212)**: the whole set in ONE item; the edit of 211 emits `HelpOfferTypesUpdated { helpOfferId, reportId, helpTypes }` (`help_offer_updated` on the timeline) | ReportManagement (timeline) |
| DirectionSightingLogged | LogDirectionSighting succeeds | `{ reportId, direction, reporterRole, loggedAt }` | DirectionSightingAggregation (reweighting). **Implemented 2026-09-04**: no event bus — the row IS the reweighting: `tb_direction_estimate`'s `ON DUPLICATE KEY UPDATE` upsert happens in the SAME transaction as the `tb_direction_sighting` insert (`direction-sightings.repository.ts insertSighting`), so there is nothing for a consumer to react to asynchronously |
...
| DirectionEstimateRepository | findByReportId, save | `DirectionEstimate`, `void`. **Implemented 2026-09-04** as `direction-sightings.repository.ts`'s `findEstimateRows`/`insertSighting` (the write side, migration 047) PLUS each consuming module's OWN copy of the read query — `reports.repository.ts getDirectionEstimateFacet` (single report) and `help-matching.repository.ts findDirectionEstimates` (batched `IN (...)` for a feed page) — table access, not a module import, the same posture as the chat/rating facets already in `reports.repository.ts` |
...
| PanicAlertRepository | create, findById — **implemented 2026-09-04** as `panic-alert.repository` `findAlertByClientKey` (idempotency, 137), `findActiveAlertByAccount` (cooldown, 198), `findAlertById`, `insertAlertWithRecipients` (alert insert + bulk recipient snapshot + read-back in ONE transaction, rolled back together on failure — amended 2026-09-06; an empty array skips the recipient statement, never the commit), `countRecipients`, `resolveAlert` (atomic `active` -> `resolved`, 197), plus `findAlertsForResponder` (the inbox JOIN, cursor by alert id, 192) | `PanicAlert`, `PanicAlertRow[]` |
...
[
  ...
  { "id": "06", "title": "Implement HelpOffer aggregate with self-dealing guard", "description": "Creates HelpOffer with HelpType selection, rejecting Reporter self-candidacy.", "scope": ["src/modules/help-offers/help-offers.service.ts", "src/modules/help-offers/__tests__/help-offers.spec.ts"], "acceptance": ["Throws SelfDealingError when helperId equals the Report's reporterId", "Accepts any non-empty, repeat-free subset of the fixed HelpType list (amended 2026-09-11, decision 208)"], "depends_on": "03" },
