...
- **Pagination (PS0, decision 220, 2026-09-21)**: the three admin lists — `GET /api/legal-policy/jurisdictions`, `GET /api/legal-policy/capabilities?jurisdiction=XX` (still mandatory, same 422 `REQUIRED` on the `jurisdiction` field) and `GET /api/legal-policy/rules?capability=&jurisdiction=` (exact filters kept) — accept optional `page` (>= 1), `pageSize` (1..100, default 20) and `filter` (trimmed, max 100; LIKE on `code, name` / `capability, description` / `capability, jurisdiction_code, legal_basis`). Without `page` the response shape is unchanged (`{ ok, data: T[] }`); with it, `data` is `{ items, page, pageSize, total }`. For the capability overview `total` counts the CATALOG rows matching the filter (the overview is one row per catalog entry, LEFT JOINed to the jurisdiction's active rule). Shared helper `src/shared/http/paged-query.ts`; specs `legal-policy.list.spec.ts`, `legal-policy.routes.spec.ts`, `legal-policy.service.spec.ts`.

## STATUS
- L0+L1 (decision 106) — DONE 2026-08-03. 26 suites / 151 tests green; `tsc` clean.
- **Wiring state** (from `src/shared/legal/capabilities.ts`, re-verified 2026-09-06): ten capabilities are WIRED — `report.anonymous` (R1, `reports.service.ts`), `report.media` (R4, attach), `reward.offer` + `reward.monetary` (Reward R0, `reward.service.ts offerReward`), `reward.intermediation.delegated` (`reserveGuarantee`), `reward.mediation` (propose/approve/execute), `chat.masked` (C1), `helper.rating` (RT1), `panic.dispatch` (PP1, `panic-alert.service.ts triggerAlert`), `location.tracking` (DS1, `direction-sightings.service.ts logDirectionSighting`). Exactly four remain in `PENDING_WIRING`: `reward.intermediation.own`, `minor.data_retention`, `identity.disclosure`, `data.cross_border`. `capabilities.catalog.spec.ts` fails the build if an entry is ever neither wired nor pending, or both — when a domain task wires a capability, it must remove the entry.
