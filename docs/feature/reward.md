...
- **Helper onboarding endpoint** that produces a `tb_reward_recipient_profile` row (the KYC data collection for the PSP subconta) — `reserveGuarantee` throws a typed `NOT_AVAILABLE` error when missing; no UI/flow exists to fill it yet.
